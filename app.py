...
st.title('Car Price Predicitor')

company = st.selectbox('select company name',sorted(company))

name = st.selectbox('select model name',sorted(name))

year = st.selectbox('select year of purchase', sorted(year))

fuel_type = st.selectbox('select fuel type',sorted(fuel_type))

kms_driven = st.number_input('enter the km run')

if st.button('Predicit Price'):
    input_data = pd.DataFrame({'name':[name],'company':[company],
                               'year':[year],'kms_driven':[kms_driven],
                               'fuel_type':[fuel_type]})
    # st.table(input_data)
    result = pipe.predict(input_data)
    result = np.round(result)
    st.header("Predicition of your car is " + str(result))